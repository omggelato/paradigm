...
(defmethod! evaluate-rules!_5x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (assert! (apply #'rules-as-variable_5x (append (list x define-environment-for-rules-fn) rules))) x)
(defmethod! evaluate-rules!_6x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (assert! (apply #'rules-as-variable_6x (append (list x define-environment-for-rules-fn) rules))) x)
(defmethod! evaluate-rules!_7x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (assert! (apply #'rules-as-variable_7x (append (list x define-environment-for-rules-fn) rules))) x)
(defmethod! evaluate-rules!_8x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
...
(defmethod! evaluate-rules-and-make-variable_8x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (multiple-value-bind (param1 param2 param3 param4 param5 param6 param7 param8) (funcall define-environment-for-rules-fn x)
    (let ((rules-list (remove nil rules)) 
          (i 0))
...
(defmethod! rules-as-variable_5x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (multiple-value-bind (param1 param2 param3 param4 param5) (funcall define-environment-for-rules-fn x)
    (let ((counter 0))
      (apply #'andv (mapcar #'(lambda (fn) 
                          ...
                          (remove nil rules))))))
(defmethod! rules-as-variable_6x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (multiple-value-bind (param1 param2 param3 param4 param5 param6) (funcall define-environment-for-rules-fn x)
    (let ((counter 0))
      (apply #'andv (mapcar #'(lambda (fn) 
                          ...
                          (remove nil rules))))))
(defmethod! rules-as-variable_7x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (multiple-value-bind (param1 param2 param3 param4 param5 param6 param7) (funcall define-environment-for-rules-fn x)
    (let ((counter 0))
      (apply #'andv (mapcar #'(lambda (fn) 
                          ...
                          (remove nil rules))))))
(defmethod! rules-as-variable_8x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (multiple-value-bind (param1 param2 param3 param4 param5 param6 param7 param8) (funcall define-environment-for-rules-fn x)
    (let ((counter 0))
      (apply #'andv (mapcar #'(lambda (fn) 
                              ...
                              (funcall fn param1 param2 param3 param4 param5 param6 param7 param8))
                          (remove nil rules))))))
(defmethod! rules-as-variable_9x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (multiple-value-bind (param1 param2 param3 param4 param5 param6 param7 param8 param9) (funcall define-environment-for-rules-fn x)
    (let ((counter 0))
      (apply #'andv (mapcar #'(lambda (fn) 
                          ...
                          (remove nil rules))))))
(defmethod! evaluate-rules!-for-all-combinations-of-2-voices_8x (x define-environment-fn &rest rules)
  :icon 161
...
(defmethod! evaluate-rules-for-all-combinations-of-2-voices-and-make-variable_8x (x define-environment-fn &rest rules)
  :icon 161
  (labels ((evaluate-rules-for-sequence-list (sequences)
             (cond (sequences
                    (append 
...
(defmethod! apply-rules-to-paradigm_8x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
  (assert! (apply #'apply-rules-and-make-variable_8x 
                  (append (list x define-environment-for-rules-fn) rules)))
  x)

(defmethod! apply-rules-and-make-variable_8x (x define-environment-for-rules-fn &rest rules) ; delete
  :icon 161
...
(defmethod! textural-motion-i (sequence &rest functions) ; delete
  :icon 324
  ; voice1
  ; voice2
  ; vertical-intervals 
...
(defmethod! interval-succession-rule (voice1 voice2 vertical-intervals voice-intervals vertical-intervals%12 input1 input2 input3)
  :icon 908
  (labels
...
(defmethod! esc-after-leap-rule (voice1 voice2 vertical-intervals voice-intervals vertical-intervals%12 input1 input2 input3)
  :icon 908            
  (labels
...
(defmethod! dissonant?v (x y)
  :icon 137
  (ommemberv (%v (om-v y x) 12) '(1 2 6 10 11)))

(defmethod! sequence->note-names (sequence)
  :icon 138
  (labels
...
(defmethod! om-seqc2timepoints-basic ((seqc list))
...
(defmethod! om-voice2timepoints-basic ((tree list))
  :initvals '(((1 2) 3) 1)    ; an initial values list
  :indoc '("" "converts duplicated integer atoms to float") ; an string list with short docs
  :icon 225 ; the icon
  :doc ""
...
(defmethod! raised7thmotion (voice1 voice2 par1 par2 par3 par4 par5 par6 mode-pcset)
  :icon 908
  (if (null mode-pcset)
      t
...
(defmethod! ms-beat-count (measures modulus)
       ...
       (pulses (pulses)
         (apply
          ...
          (mapcar #'(lambda (p)
                  ...
                  pulses))))
    (tree tree)))
(defmethod! process-measure-durations (ms durations &key enp list-mode proportional-mode)
...
(defmethod! open-intervals-1 (list)
  :icon 908
  (labels
