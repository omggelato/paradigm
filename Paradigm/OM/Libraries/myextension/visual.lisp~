...
(defmethod! ?a-member-of (sequence) (screamer:a-member-ofv sequence))
...
(defmethod! ?< (&rest xs) :icon 255 (apply #'t2l::om<v xs))
(defmethod! ?> (&rest xs) :icon 256 (apply #'t2l::om>v xs))
(defmethod! ?<= (&rest xs) :icon 255 (apply #'t2l::om<=v xs))
(defmethod! ?>= (&rest xs) :icon 256 (apply #'t2l::om>=v xs))
(defmethod! ?= (&rest xs) :icon 259 (apply #'t2l::om=v xs))
(defmethod! ?/= (&rest xs) :icon 260 (apply #'t2l::om/=v xs))
(defmethod! ?equal (x y) :icon 259 (t2l::omequalv x y))
(defmethod! ?between (x min max) :icon 235 (apply #'?and (map-func #'(lambda (y) (and y (?and (or (null min) (?>= y min)) (or (null max) (?<= y max))))) (if (listp x) x (list x)))))
...
(defmethod! fuseseqc (sequence1 sequence2)
  :icon 230
  (cond
   ((null sequence1) sequence2)
   ((null sequence2) sequence1)
   (T
    (assert (= (length sequence1) (length sequence2)))
    (mapcar #'(lambda (xs ys) (append xs ys)) sequence1 sequence2))))

(defmethod! ?crossw (key variable map) ; delete
  :icon 147
  (labels
      ((1st (xs) (car xs))
       (2nd (xs) (cond
                  ...
                  (T (cdr xs)))))
    (map?or #'(lambda (entry)
                (?and (?equal key (1st entry))
                      (funcall (2nd entry) variable)))
            map)))

(defmethod! ?funcall (fn &rest x) :icon 147 (apply #'screamer:funcallv (append (list fn) x)))

(defmethod! ?apply (fn &rest x) :icon 147 (apply #'screamer:applyv (append (list fn) x)))

(defmethod! ?counttrues (&rest xs)
  :icon 235 
  (apply #'screamer:count-truesv (if (listp xs) (flatt xs) (list xs))))

(defmethod! symxlat (x map) :icon 147 (t2l::symxlat x map))

(defmethod! ?symxlat (x map &optional function-input use-backward-chaining)
  :icon 147
  (cond
   (function-input 
    (t2l::?symxlat-in-function-mode x map function-input use-backward-chaining))
   (T 
    (?symxlat-internal x map))))
...
(defmethod! print-solver-output (&optional catalog)
  :icon 129
  (mapcar #'print (t2l::solver-output catalog))
  (length (t2l::solver-output catalog)))
...
(defmethod! next-solver-output (&optional catalog)
  :icon 215
  (let ((catalog (or catalog :backup)))
    (cond
     ((null (find catalog (mapcar #'car t2l::*backup-solver-output*))) nil)
     ((null (cdr (assoc catalog t2l::*backup-solver-output*))) nil)
     (T
      (let ((next (car (cdr (assoc catalog t2l::*backup-solver-output*)))))
        (rplacd (assoc catalog t2l::*backup-solver-output*)
                (cdr (cdr (assoc catalog t2l::*backup-solver-output*))))
        next)))))
